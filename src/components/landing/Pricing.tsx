...
const Pricing = () => {
  ...
  const formatPrice = (plan: Plan) => {
    if (plan.isCustom || plan.price == null) return "Em breve";
    return plan.price.toLocaleString("pt-BR", { minimumFractionDigits: 2 });
  };
              ...
              >
                ...
                <div className="text-center mb-8">
                  ...
                  <div className="flex items-baseline justify-center gap-1">
                    {plan.price != null && !plan.isCustom && <span className="text-sm text-muted-foreground">R$</span>}
                    <span className={`font-extrabold group-hover:text-primary transition-colors ${plan.isCustom || plan.price == null ? "text-3xl" : "text-5xl"}`}>
                      {formatPrice(plan)}
                    </span>
                    {plan.price != null && !plan.isCustom && <span className="text-muted-foreground">/mês</span>}
                  </div>
                </div>
